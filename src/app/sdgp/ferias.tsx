...
export default function FeriasScreen() {
  ...
  const info = useMemo(() => {
    ...
    if (a < anoAtual || a > anoAtual + 1) {
      return { erro: `Use uma data de ${anoAtual} ou ${anoAtual + 1}.`, fim: '', retorno: '' };
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TouchableOpacity
          ...
          onPress={() => Alert.alert('Demonstração', 'Solicitação simulada. Nenhum dado foi enviado.')}
        >
          <Text style={styles.btnText}>Simular Solicitação</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  cardLegal: { backgroundColor: GOV_COLORS.azulClaro, padding: 15, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: '#BFDBFE' },
  linhaResumo: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  divisor: { height: 1, backgroundColor: '#BFDBFE', marginVertical: 12 },
