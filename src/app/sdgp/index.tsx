...
export default function SdgpHome() {
  ...
  const { nome, cargo, uorg, matricula } = servidor || {};
  ...
  useEffect(() => {
    const buscarResumoFinanceiro = async () => {
      if (isDemoCpf(servidor?.cpf)) {
        setUltimoContracheque(DEMO_CONTRACHEQUES[0]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.contrachequeCard}>
          ...
          <View style={styles.ccBotaoRow}>
            <TouchableOpacity 
              ...
              onPress={() => router.push('/sdgp/contracheque')}
            >
        ...
        <View style={styles.serviceGroup}>
          ...
          <ServiceCard icon="palm-tree" title="Férias e Recessos" subtitle="Programar, alterar ou consultar" color="#00BCD4" onPress={() => router.push('/sdgp/ferias')} />
          <ServiceCard icon="calculator-variant" title="Simular Aposentadoria" subtitle="Tempo de contribuição e regras" color="#9C27B0" onPress={() => router.push('/sdgp/aposentadoria')} />
        </View>
...
const styles = StyleSheet.create({ 
  ...
  scrollContent: { padding: 15, paddingBottom: 30, width: '100%', maxWidth: 880, alignSelf: 'center' },
