...
const NOTICIAS_API = [
  'Demonstração: explore os módulos usando apenas os dados fictícios.',
  'Projeto N2: solicitações criadas no modo demo não são enviadas.',
  'Segurança: não informe CPF, senha ou dados funcionais reais.'
];
...
const MODULOS_ATIVOS = [
  ...
  { id: 'ia-copilot', titulo: 'Assistente N2', subtitulo: 'Resposta simulada para o protótipo', icone: 'auto-awesome', rota: '/ia-copilot', badge: 'Demo', badgeCor: '#FFCD00' }
];

export default function Dashboard() {
  const router = useRouter();
  const { carregando: carregandoSessao, servidor, sair } = useSession();
  const nomeServidor = servidor?.nome || 'SERVIDOR AUTENTICADO';
  const cargoServidor = servidor?.cargo || 'Cargo não informado';
  const uorgServidor = servidor?.uorg || 'Lotação não informada';
  const cpfServidor = servidor?.cpf;
  ...
  useEffect(() => {
    const buscarSalario = async () => {
      ...
      if (isDemoCpf(servidor.cpf)) {
        setUltimoSalario(DEMO_CONTRACHEQUES[0].liquido);
        setMesSalario(DEMO_CONTRACHEQUES[0].mes_referencia);
        setCarregandoSalario(false);
        return;
      }
      try {
        const { data, error } = await supabase
          ...
          .eq('matricula', servidor.matricula)
  ...
  if (!servidor) return <Redirect href="/login" />;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.govBar}>
        <View style={styles.logoContainer}>
          <Text style={styles.govText}>Portal N2</Text>
          <View style={styles.separadorVertical} />
          <Text style={styles.subTituloGov}>Ambiente acadêmico</Text>
        </View>
        <TouchableOpacity
        ...
        >
          <MaterialIcons name="logout" size={18} color="#FFCD00" />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.painelUsuario}>
          ...
          <View style={styles.seloVerificacao}>
            <MaterialIcons name="science" size={14} color="#FFCD00" />
            <Text style={styles.seloTexto}>Sessão demonstrativa</Text>
          </View>
        </View>
        ...
        <View style={styles.conteudoSessao}>
          ...
          {MODULOS_ATIVOS.map((mod) => (
            <TouchableOpacity 
              ...
              activeOpacity={0.85}
              onPress={() => router.push(mod.rota as any)}
            >
        ...
        <View style={styles.rodapeGov}>
          <Text style={styles.rodapeTexto}>Projeto N2 — protótipo acadêmico</Text>
          <Text style={styles.rodapeSub}>Dados exclusivamente demonstrativos</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({ 
  ...
  scrollContent: { paddingBottom: 40, width: '100%', maxWidth: 880, alignSelf: 'center' },
