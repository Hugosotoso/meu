...
export default function ContrachequeScreen() {
  ...
  const { nome, cargo, matricula, cpf } = servidor || {};
  ...
  useEffect(() => {
    const buscarDados = async () => {
      if (isDemoCpf(cpf)) {
        setContracheques(DEMO_CONTRACHEQUES);
        setCarregando(false);
        return;
      }
      ...
      try {
        const { data, error } = await supabase
          .from('contracheques')
          .select('*')
          .eq('matricula', String(matricula)); // Garante que a matrícula vai como Texto
        ...
        Alert.alert('Erro', 'Falha ao conectar com o serviço de dados.');
      } finally {
        setCarregando(false);
      }
    };
      ...
      const htmlPDF = `
        <html>
          ...
          <body>
            ...
            <div class="content">
              <div class="title">Resumo de Rendimentos - ${escaparHtml(dados.mes_referencia)}</div>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.rodapeAcoes}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => router.push('/sdgp/ficha')}
        >
