/**
 * Super App Gov — Assistente Virtual GOV.ia (BLINDADO FINAL)
 */
...
export default function IaCopilotScreen() {
  ...
  const { nome, cargo, uorg } = useLocalSearchParams();
  const scrollRef = useRef<ScrollView>(null);

  const [mensagens, setMensagens] = useState<Mensagem[]>([
    { 
      id: '1', 
      texto: `Olá, ${nome || 'Servidor'}! Sou o GOV.ia. Como atuante no cargo de ${cargo || 'servidor'} na unidade ${uorg || 'central'}, de que forma posso agilizar o seu expediente hoje?`, 
      isUser: false 
    }
  ]);
  ...
  const enviarMensagem = async () => {
    ...
    setTimeout(() => {
      setMensagens((prev) => [...prev, { id: Date.now().toString(), texto: `Olá, ${nome || 'Servidor'}. Como ${cargo || 'servidor'} da unidade ${uorg || 'central'}, informo que a sua solicitação foi processada com sucesso via rede interna. A prioridade está definida como máxima.`, isUser: false }]);
      setCarregando(false);
    }, 1200);
  };
