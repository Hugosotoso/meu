...
const CATALOGO = [
  ...
  { id: '12', nome: 'Pendrive 32GB', icone: 'usb', categoria: 'Informática' },
];
...
export default function AlmoxarifadoScreen() {
  ...
  const handleFinalizarPedido = async () => {
    ...
    if (isDemoCpf(cpf)) {
      setHistorico((atual) => [{ id: `DEMO-${Date.now()}`, itens: itensFormatados, status: 'Simulado' }, ...atual]);
      setCarrinho({});
      setCarregando(false);
      Alert.alert('Demonstração', 'Pedido simulado. Nenhum dado foi enviado.');
      return;
    }
